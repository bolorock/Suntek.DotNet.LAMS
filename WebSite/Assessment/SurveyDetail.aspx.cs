...
namespace WebSite
{
    public partial class SurveyDetail : BasePage
    {
       	...
       	protected bool ShowPageDetail()
        {
            ...
            if (!string.IsNullOrWhiteSpace(entity.SurveyTarget))
            {
                IList<string> surveyTargets = entity.SurveyTarget.Trim(',').Split(',').ToList();
              ...
              chbSurveyTarget.Text = surveyTargetStr.ToSafeString().Trim(',');
              chbSurveyTarget.Value = entity.SurveyTarget;


            //取得角色配置的信息

            DataTable dt = new AssessmentRoleWeightService().GetRoleBySurveyID(entity.ID);
            string roleStr = string.Empty;
            foreach (DataRow dr in dt.Rows)
            {
                    roleStr += String.Format("{0}:{1}%; ", dr["Value"], dr["Weight"]);
            }
            if (!string.IsNullOrWhiteSpace(roleStr))
            {
                litRole.Text = roleStr;
            }
                ...
                if (!string.IsNullOrWhiteSpace(userIDs))
                {
                    ...
                    foreach (string para in parameters)
                    {
                        ...
                        surveyResult = new SurveyResult()
                        {
                            ...
                            SurveyID = CurrentId,
                            Tester = spara[0],
                            AssessmentRoleID = spara[1] == "undefined" ? "" : spara[1],
                            IsLock = 0
                        };
                        surveyResults.SafeAdd(surveyResult);
                    }
        ...
        public string ChangeLock(string argument)
        {
            ...
            try
            {
                ...
                if (surveyResult != null)
                {
                    ...
                    surveyResultService.ChangeLock(surveyResultID, isLock);
                   
                    actionResult = ActionResult.Success;
                }

                if (isLock == 0)  // 如果是解锁的话，则再进行待办的发送
                {

                    Survey survey = surveyService.GetDomain(surveyResult.SurveyID);

                    #region 发待办给待评人

                    EIACMessage eIACMessage = new EIACMessage()
                    {
                        Title = string.Format("测评 - {0}", survey.Title),
                    ...
                    {
                        AppWorkQueueID = surveyResult.ID,
                        Executor = employee.LoginName,
                        ExecutorName = employee.Name,
                        CurrActivity = "测评",
                        PendingItemURL = string.Format("{0}Assessment/SurveyManager.aspx?Entry=test", RootPath),
                    };
        ...
        public string Publicshed(string argument)
        {
            ...
            try
            {
                ...
                if (survey != null)
                {
                    ...
                    DataTable dt = surveyResultService.GetSurveyResultAndUser(argument);
                    foreach (DataRow dr in dt.Rows)
                    {
                        EIACWorkItem workItem = new EIACWorkItem()
                        {
                            ...
                            CurrActivity = "测评",
                            PendingItemURL = string.Format("{0}Assessment/SurveyManager.aspx?Entry=test&SurveyID={1}", RootPath, argument),
                        };
                        eIACMessage.WorkItems.Add(workItem);
                    }
