...
namespace WebSite
{
    public partial class Uploadify : BasePage
    {
        ...
        private bool SaveSurveyResult(HSSFSheet sheet, string templatePath, int rowCount, string surveyID)
        {
            ...
            SurveyResultModel model = SunTek.LAMS.Uitl.ExcelUtil.CreateFromExcel<SurveyResultModel>(templatePath, (row, column) =>
            {
                ...
                {
                    log.Error(string.Format("获取导入评估人excel单位格信息出错!【行，列】＝【{1],{2}】", row, column), ex);
                    return string.Empty;
                }
            }, rowCount);
            ...
            model.AssessmentRoleID = Request["roleID"];
            ...
            SurveyResultService surveyResultServic = new SurveyResultService();
            surveyResultServic.SaveImportSurveyResult(model, out strError);
