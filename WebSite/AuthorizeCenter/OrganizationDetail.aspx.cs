...
namespace WebSite
{
    public partial class OrganizationDetail : BasePage
    {
       	...
       	protected bool ShowPageDetail()
        {
			if (PageContext.Action == ActionType.Add)
			{
				...
				dtpModifier.Text=DateTime.Now.ToShortDateString();
				dtpModifyTime.Text=DateTime.Now.ToShortDateString();
				return false;
			}

            Organization entity =organizationService.GetDomain(CurrentId);
		...
		{
            Organization  organization = organizationService.GetDomain(CurrentId);
			...
			GetControlValues(ref organization);	
