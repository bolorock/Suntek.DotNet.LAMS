...
namespace WebSite
{
    public partial class ExamPaperManager : BasePage
    {
        ...
        protected  void Redirect(string param)
        {
            ...
            Response.Redirect(string.Format("ExamPaperDetail.aspx?LastUrl={0}&Runat=1&ActionFlag={1}{2}{3}", Request.Url.PathAndQuery, PageContext.Action, currentIdParam, string.IsNullOrEmpty(param) ? param : "&" + param));
        }
        ...
        {
            IPageOfList<ExamPaper> result = examPaperService.FindAll(GetFilterParameters(), pageInfo);
        ...
        public string Delete(string argument)
        {
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(argument))
                {
                    ...
                    para.SafeAdd("ExamPaperID", argument);
                    ...
                    if (result != null && result.Count > 0)
                    {
                        ...
                        ajaxResult.PromptMsg = "已发布关联该问卷的测评活动，无法进行删除！";
                        return JsonConvert.SerializeObject(ajaxResult);
                    }


                    examPaperService.Delete(argument);
        ...
        public string Copy(string argument)
        {
            AjaxResult ajaxResult = new AjaxResult();
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(argument))
                {
                    examPaperService.Copy(argument);
                    actionResult = ActionResult.Success;
        ...
        /// 保存测评活动问卷选择
        /// </summary>
        /// <param name="argument">问卷id</param>
        /// <returns></returns>
        public string SaveChoose(string argument)
        {
            AjaxResult ajaxResult = new AjaxResult();
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(argument)&&!string.IsNullOrWhiteSpace(surveyId))
                {
                    
                    SurveyService surveyService = new SurveyService();
                    Survey survey = surveyService.GetDomain(surveyId);
                    survey.ExamPaperID = argument;
