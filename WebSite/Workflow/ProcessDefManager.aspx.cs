...
namespace WebSite
{
    public partial class ProcessDefManager : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                IList<DictItem> dictItems = dictService.GetDictItems("ProcessCategory");
        ...
        public void Delete()
        {
            ...
            try
            {
                processDefService.Delete(CurrentId);
        ...
        {
            IPageOfList<ProcessDef> result = processDefService.FindAll(GetFilterParameters(), pageInfo);
        ...
        public string Publish(string argument)
        {
            ...
            try
            {
                ...
                ProcessDef processDef = processDefService.GetDomain(argument);
                if (processDef != null)
                    processDef.CurrentState = 1; //已发布


                processDefService.SaveOrUpdate(processDef);
                processDefService.ClearCache();
                actionResult = ActionResult.Success;
                ...
                ajaxResult.RetValue = RemarkAttribute.GetEnumRemark((EAFrame.Workflow.Enums.ProcessStatus)Enum.Parse(typeof(EAFrame.Workflow.Enums.ProcessStatus), processDef.CurrentState.ToString()));
                ajaxResult.PromptMsg = actionMessage;
            }
            catch (Exception ex)
            {
        ...
        public void ActivityManager()
        {
            ...
            Response.Redirect(string.Format("ActivityManager.aspx?ProcessDefID={0}&Runat=1&Entry=Manager&LastUrl={1}", CurrentId, Request.Url.PathAndQuery));
        }
