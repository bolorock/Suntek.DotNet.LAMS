    ...
    public partial class SurveyManager : BasePage
    {
        ...
        public void ShowList(PagedGridView gvList, PageInfo pageInfo)
        {
            ...
            if (Request.QueryString["Entry"].EqualIgnoreCase("test"))
            {
                ...
                para.SafeAdd("ID", new Condition(string.Format("ID in (select SurveyID from LA_SurveyResult r where r.Tester='{0}' and r.Status=0) and Status=1", User.ID)));
            }

            if (Request.QueryString["Entry"].EqualIgnoreCase("finished"))
            {
                // para.SafeAdd("Status", 2);
                para.SafeAdd("ID", new Condition(string.Format("ID in (select SurveyID from LA_SurveyResult where Tester='{0}' and Status={1})", User.ID, 2)));
            }
